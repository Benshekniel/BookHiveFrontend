...
const LendOnlyList = () => {
	...
	const getLendOnlyList = async () => {
		if (!user?.userId) return [];
		try {
			const response = await axios.get(`http://localhost:9090/api/bs-book/lendOnlyList/${user.userId}`);
	...
	const { data: lendOnlyList = [], isPending, error } = useQuery({
						...
						<table className="w-full">
							<thead className="bg-slate-50 border-b border-gray-200">
								<tr>
									...
									<th className="p-4 font-semibold text-slate-700 whitespace-nowrap">LEND FEE</th>
							...
							<tbody>
								{isPending ? (
									<tr>
										...
										</tr>) : lendOnlyList.length === 0 ? (
											<tr>
												<td colSpan={9} className="text-center py-6 text-gray-400">
													No books found.
												</td>
											</tr>) :
									(lendOnlyList.filter((book) => {
