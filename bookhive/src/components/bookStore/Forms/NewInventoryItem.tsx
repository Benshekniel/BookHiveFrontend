...
const NewInventoryItem = () => {
  ...
  const createInventory = async (newItemData: InventoryFormFields, coverImage: File) => {
    const formData = new FormData();

    const dataExtended = { ...newItemData, userId: user.userId, isForDonation: isForDonation };
    const jsonBlob = new Blob([JSON.stringify(dataExtended)], {
      type: "application/json",
    });

    formData.append("newItemData", jsonBlob);
    formData.append("coverImage", coverImage);

    const response = await axios.post("http://localhost:9090/api/bs-inventory/new", formData, {
            ...
            <form onSubmit={handleSubmit(onSubmit, onError)} className="p-6 space-y-4">
              ...
              <div className="sticky bottom-0 z-10 bg-white border-t border-gray-200 flex justify-center p-4">
                <button type="submit"
                  ...
                  {createMutation.isPending ? <LoadingSpinner /> : "Add Inventory"}
                </button>
              </div>
            </form>
          </div>
