...
const DeleteInventory = ({inventoryId}: {inventoryId: number}) => {
  const queryClient = useQueryClient();
    // const { user } = useAuth();
  const user = { userId: 603 };

  const deleteItem = async (inventoryId: number) => {
    const response = await axios.delete(`http://localhost:9090/api/bs-inventory/${inventoryId}`);
    return response.data;
  }

  const deleteMutation = useMutation({
    mutationFn: ({ inventoryId }: { inventoryId: number }) => deleteItem(inventoryId),
    onMutate: () => {
  ...
  const deleteItemConfirm = () => {
    Swal.fire({
      ...
      if (result.isConfirmed && inventoryId !== null) {
        deleteMutation.mutate({ inventoryId: inventoryId });
      }
    });
  }
  ...
  return (
    <button className="p-2 bg-red-100 border border-red-200 hover:bg-red-200 rounded-lg transition-colors duration-200"
      onClick={deleteItemConfirm} title="Delete inventory item">
