...
const Messages = () => {
  ...
  const handleSocketMessage = useCallback(
    (data) => {
      ...
      console.log('Current user ID:', orgId);
      ...
      if (type === 'new_message') {
        ...
        // Ignore messages sent by current user to prevent duplicates
        if (senderId === orgId) {
  ...
  useEffect(() => {
    if (user && orgId) {
      console.log('Connecting to Socket.IO...', orgId);
  ...
  }, [orgId, handleSocketMessage, handleUnreadCountUpdate, handleConversationUpdate, handleConnectionChange]);
  ...
  const fetchUnreadCount = async () => {
    try {
      const count = await messageService.getUnreadMessageCount(orgId);
  ...
  const fetchAvailableContacts = async () => {
    try {
      ...
      const contacts = await messageService.getAvailableContactsByRole(user.role, orgId);
      const filteredContacts = contacts.filter((contact) => allowedRoles.includes(contact.role.toLowerCase()));
  ...
  const fetchConversations = async () => {
    try {
      ...
      console.log('Fetching conversations for user:', orgId);

      const userMessages = await messageService.getUserMessages(orgId);
      console.log('User messages:', userMessages);

      const conversationMap = new Map();

      userMessages.forEach((message) => {
        const partnerId = message.senderId === orgId ? message.receiverId : message.senderId;
        const partnerName = message.senderId === orgId ? message.receiverName : message.senderName;
        ...
        if (message.receiverId === orgId && !message.Read) {
          conversation.unread++;
        }
      });
      ...
      await loadConversationMessages(conversationsArray, orgId);
    } catch (err) {
  ...
  const markConversationAsRead = async (conversationId) => {
    try {
      console.log(`Marking conversation ${conversationId} as read for user ${orgId}`);

      await messageService.markConversationAsRead(orgId, conversationId);
  ...
  // Redirect to login if not authenticated (redundant due to earlier check, but kept for consistency)
  if (!user) {
      ...
      {error && (
        <div
          className={`mb-4 p-3 border-l-4 ${
            error.includes('success') ? 'bg-green-100 border-green-500 text-green-700' : 'bg-red-100 border-red-500 text-red-700'
          }`}
        >
          {error}
        </div>
                ...
                >
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs mt-1 font-medium ${getRoleColor(conversation.role)}`}>{conversation.role}</p>
                    </div>
                  </div>
                </div>
              ))
