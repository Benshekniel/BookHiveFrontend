...
const BooksPage = () => {
  ...
  const bookLocations = books.map((book) => ({
    ...
    lat: 6.9271 + (Math.random() - 0.5) * 0.05, // Random offset around Colombo
  ...
  const applyAIRecommendations = useCallback(() => {
    ...
    if (aiSearchQuery) {
      ...
      // Default to popular books if no AI search query
      result = [...books]
  ...
  const handleBorrowConfirm = (book) => {
    alert(
      `Your borrow request for "${book.title}" was successfully sent to the seller.`
    );
  };

  const handleExchangeConfirm = (book) => {
    alert(
      `Your exchange request for "${book.title}" was successfully sent to the owner.`
    );
  };
          ...
          <>
                ...
                <div className="bg-white/90 rounded-2xl shadow-md border border-gray-200 p-4 sm:p-6">
                  ...
                  <div className="mt-6">
                    ...
                    <div className="relative h-64 sm:h-80 rounded-lg overflow-hidden border border-gray-200">
                      <MapContainer
                        center={[6.9271, 79.8612]} // Centered on Colombo
                        zoom={12}
                      ...
                      >
                        ...
                        {nearbyOnly &&
                          bookLocations
                            .filter((loc) =>
                              loc.location.toLowerCase().includes("colombo")
                            )
                            .map((loc) => (
                              <Marker
                                key={loc.id}
                      ...
                      <div className="text-center py-12 bg-white/90 rounded-lg shadow-md border border-gray-200">
                        ...
                        {activeFiltersCount > 0 && (
                          <Button
                            variant="outline"
                            onClick={clearAllFilters}
                            className="border border-gray-300 text-gray-600 hover:bg-gray-100 text-sm px-4 py-1 rounded mt-3"
                          >
                            Clear all filters
                          </Button>
                        )}
            ...
            {showWishlistAddModal && (
              <div className="fixed inset-0 flex items-center justify-center z-50 bg-transparent bg-opacity-50 shadow-2xl">
                <div className="bg-white/95 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto shadow-lg p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3
