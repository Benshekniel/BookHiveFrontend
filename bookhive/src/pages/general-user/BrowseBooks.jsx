...
const placeholder =
  "data:image/svg+xml,%3csvg width='150' height='200' xmlns='http://www.w3.org/2000/svg'%3e%3crect width='150' height='200' fill='%236B7280'/%3e%3ctext x='75' y='100' text-anchor='middle' fill='%23FFFFFF' font-size='14'%3eNo Image%3c/text%3e%3c/svg%3e";
...
const BooksPage = () => {
        ...
        if (!cachedImage) {
          try {
            const response = await fetch(
              `${baseUrl}/getFileAsBase64?fileName=${book.bookImage}&folderName=userBooks`
            );
            if (response.ok) {
    ...
    try {
      ...
      const mappedBooks = response.data.map((book, index) => ({
        ...
        owner: {
          ...
          avatar: book.userAvatar || null,
          profileImage: book.userProfileImage || null,
  ...
  const handleBorrowPayment = (request) => {
    setSelectedBook({ title: request.bookName, price: request.lendPrice || 0 });
    setPaymentType("borrow");
    setShowPaymentModal(true);
  };

  const handleExchangePayment = (request) => {
    setSelectedBook({ title: request.bookName, price: 0 });
  ...
  const processPayment = () => {
    const amount =
      paymentType === "borrow"
        ? Math.round((selectedBook?.price || 0)) + 200
        : 200;
  ...
  // Delete (optimistic — plug your real DELETE when ready)
  const handleDeleteBorrowRequest = async (requestId, bookId) => {
    const confirmed = window.confirm("Delete this request? This action cannot be undone.");
    if (!confirmed) return;

    try {
      setDeletingReq((p) => ({ ...p, [requestId]: true }));
      const res = await fetch(`${baseUrl}/api/deleteBorrowRequest/${bookId}/${requestId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
      ...
      setBorrowRequests((prev) =>
        prev.filter((r) => !(r.requestId === requestId && r.bookId === bookId))
      );
      ...
      console.error("Delete borrow request error:", e);
      showToast("Failed to delete. Please try again.", "error");
    } finally {
      setDeletingReq((p) => ({ ...p, [requestId]: false }));
    }
  };
                ...
                <div className="bg-white/90 rounded-2xl shadow-md border border-gray-200 p-4 sm:p-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 mb-4">
                    ...
                    <button
                      ...
                      className={`bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg flex items-center space-x-2 text-sm transition-colors duration-200 ${showFilters ? "bg-blue-600" : ""}`}
                    >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                    ...
                    <div className="lg:col-span-1">
                      ...
                      <div className="relative h-48 sm:h-64 rounded-lg overflow-hidden border border-gray-200">
                        <MapContainer center={userPosition} zoom={12} scrollWheelZoom={true} className="w-full h-full z-0">
                          <TileLayer
                          ...
                          <div className="p-3">
                            ...
                            <div className="mt-2 flex space-x-1">
                              <Button
                                ...
                                onClick={() => setWishlistedBooks((prev) => prev.filter((b) => b.id !== book.id))}
                              >
              ...
              <div className="bg-white/90 rounded-2xl shadow-md border border-gray-200 p-4 sm:p-6">
                ...
                {loadingRequests ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 text-sm">Loading your requests...</p>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    return (
                      <div key={req.requestId} className="bg-white rounded-lg border border-gray-200 p-4">
                          ...
                          <div className="flex items-start">
                            ...
                            {req.bookImageName ? (
                              <LazyImage
                                ...
                                fileName={req.bookImageName}
                                folderName="userBooks"
                                baseUrl={baseUrl}
                              />
                            ) : (
                              <div className="w-14 h-18 bg-gray-200 rounded mr-3 flex items-center justify-center">
                                <BookOpen className="w-5 h-5 text-gray-400" />
                              </div>
                            )}

                            <div>
                              <h3 className="font-semibold text-gray-900">{req.bookName}</h3>
                              <p className="text-sm text-gray-600">by {req.author}</p>
                              <p className="text-xs text-gray-500">
                                Requested: {req.createdAt ? new Date(req.createdAt).toLocaleString() : "-"}
                              </p>
                              <p className="text-xs text-gray-500 mt-1">
                                Owner: {req.ownerName} • Lend: Rs. {req.lendPrice ?? 0} • Delivery: Rs. {req.deliveryPrice ?? 0} • Handling: Rs. {req.handlingPrice ?? 0}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {status === "APPROVED" && (
                            <Button
                              ...
                              onClick={() => handleBorrowPayment(req)}
                            >
                              Make Payment
                            </Button>
                          )}
                          ...
                          <Button
                            ...
                            onClick={() => handleDeleteBorrowRequest(req.requestId, req.bookId || req.book_id)}
                            disabled={!!deletingReq[req.requestId]}
                          >
                            {deletingReq[req.requestId] ? "Deleting..." : "Delete"}
                          </Button>
                        </div>
                      </div>
                    )})}
                ...
                <div className="bg-white rounded-2xl max-w-md w-full mx-4 p-6">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="bg-gray-50 rounded-lg p-3">
                      ...
                      <p className="text-sm text-gray-600">Borrowing Price: Rs. {Math.round(selectedBook?.price * 0.2)}</p>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 mb-4">
                      ...
                      <div className="space-y-2 text-sm">
                        {paymentType === "borrow" && (
                          <>
                            <div className="flex justify-between">
                              ...
                              <span>Rs. {Math.round(selectedBook?.price || 0)}</span>
                            </div>
                          </>
                        )}
                        <div className="flex justify-between">
                        ...
                        <div className="border-t pt-2 flex justify-between font-semibold">
                          ...
                          <span>
                            Rs. {paymentType === "borrow" ? Math.round(selectedBook?.price || 0) + 200 : 200}
                          </span>
                        </div>
                      </div>
                    </div>
