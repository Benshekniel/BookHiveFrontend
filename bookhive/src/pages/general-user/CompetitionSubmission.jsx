...
export const SubmissionEditor = ({ initialContent, onChange, wordCount, disabled }) => {
  ...
  const editorConfig = useMemo(
    () => ({
      ...
      toolbar: 'undo redo | formatselect | bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | fontsize | removeformat',
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mt-2 text-sm text-gray-500">
        ...
        <span>Follow competition rules</span>
      </div>
    </div>
  );
};
...
const CompetitionSubmission = ({
      ...
      if (selectedCompetition && user && !draftLoaded) {
        try {
          ...
          if (response.data?.message === 'success') {
            ...
            editorContentRef.current = draftSubmission.content || "";
          } else {
          ...
          setSubmitError(`Failed to fetch draft: ${error.message}`);
        } finally {
          setIsLoading(false);
        }
      }
  ...
  // Handle content change without debouncing
  ...
  const handleClose = useCallback(() => {
    ...
    setSaveSuccess(false);
  }, [setShowSubmissionModal, setSelectedCompetition]);

  const handleSubmitEntry = async () => {
    if (selectedCompetition && submissionForm.title && editorContentRef.current) {
      setIsSubmitting(true);
      setSubmitError(null);
      try {
        const submissionData = {
          submissionId: draftSubmissionId || null,
          competitionId: selectedCompetition.id,
          email: user.email,
      ...
      setSubmitError('Please provide both a title and content.');
      setIsSubmitting(false);
    }
  };

  const isDeadlinePassed = useCallback((deadline) => {
        ...
        <div className="p-6">
          ...
          <div className="mt-6 flex justify-end space-x-3">
            ...
            <Button
              ...
              disabled={!submissionForm.title || !editorContentRef.current || isDeadlinePassed(selectedCompetition.deadline) || isSubmitting || isLoading || saveSuccess}
              icon={<Send size={16} />}
            >
              {isSubmitting ? (
                <>
...
// Button component
const Button = ({ children, variant = "primary", icon, onClick, disabled = false }) => {
  ...
  return (
    <button className={`${baseClasses} ${variants[variant]} px-4 py-2 text-sm`} onClick={onClick} disabled={disabled}>
