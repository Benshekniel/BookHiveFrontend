    ...
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2 text-lg">Review and moderate book listings and user content</p>
        </div>
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {bookListings.map((book) => (
                    <tr key={book.id} className="hover:bg-blue-50 transition-colors duration-200">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div className="text-sm text-gray-900 font-medium">{book.owner}</div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-900 font-medium">{book.condition}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          ...
                          <span className={`text-sm font-medium ${book.reportCount > 0 ? 'text-red-600' : 'text-gray-500'}`}>
                            {book.reportCount}
                          </span>
                        </div>
                      </td>
          ...
          <div className="p-6">
            <div className="space-y-4">
              {reportedContent.map((report) => (
                <div key={report.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow duration-200 bg-white">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 mb-1">Item: {report.itemName}</p>
                      <p className="text-sm text-gray-600 mb-1">Reported by: {report.reportedBy}</p>
                      <p className="text-sm text-gray-600 mb-1">Reason: {report.reason}</p>
                      <p className="text-xs text-gray-400">Reported on: {report.date}</p>
                    </div>
                    <div className="flex items-center gap-2 ml-4">
                      <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors duration-200 shadow-sm">
                        Review
                      </button>
                      <button className="px-4 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 transition-colors duration-200 shadow-sm">
                        Approve
                      </button>
                      <button className="px-4 py-2 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 transition-colors duration-200 shadow-sm">
                        Remove
                      </button>
                    </div>
                  </div>
